/*
Exercícios Práticos
1.​ Melhorando a Busca: Atualmente, a busca pelo itemSelecionado no
método main usa o ID como um índice de array (id-1). 
Modifique este trecho para usar o laço for que foi demonstrado na aula, que busca o item
comparando o item.id. 

Essa abordagem é mais segura, pois não assume
que os IDs serão sempre sequenciais a partir de 1.

2.​ Filtrando por Categoria: Adicione um novo método à classe Cardapio chamado 
 - imprimeItensPorCategoria(CategoriaCardapio categoria).
Este método deve receber uma categoria como parâmetro e imprimir no console
	 - nome 
	 - preço 
	 - de todos os itens que pertencem a essa categoria. 
	 Chame este método no main para testá-lo, 
	 por exemplo, com CategoriaCardapio.BEBIDAS.
*/
void main() {
	...
	ItemCardapio itemSelecionado = cardapio.buscaPorId(idSelecionado);
	...
	cardapio.imprimeItensPorCategoria(CategoriaCardapio.SOBREMESAS);
...
class Cardapio {
	...
	ItemCardapio buscaPorId(long idProcurado) {
		for (ItemCardapio item: itens) { // Percorre cada item do array
			if (item !=null && item.id == idProcurado) {// Verifica se o ID bate
				return item; // Se achou, retorna o objeto e encerra a busca
			}
		}
	...
	double obtemPrimeiroPrecoMaiorQueLimite(double precoLimite) {
		...
		void imprimeItensPorCategoria(CategoriaCardapio categoria) {
		for (ItemCardapio item : itens) {
			if (item != null && item.categoria == categoria) {
				IO.println("Nome: " + item.nome);
				IO.println("Preço: " + item.preco);
				IO.println("---");
			}
		}
	}
